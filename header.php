...
<html <?php language_attributes(); ?>>
    ...
    <body <?php body_class(); ?>>
        ...
        <?php
            ...
            $on_front_page          = get_theme_mod( 'nandgatestudios-header-front-page', true );
            ...
            // blog page ( show / hide )
            $on_blog_page           = get_theme_mod( 'nandgatestudios-header-blog-page', false );
            ...
            // check if is single post
                ...
                $show_header = get_theme_mod( 'nandgatestudios-header-single-post', false );
            }
            ...
            // check if is single page ( exclude static front page )
                ...
                $show_header = get_theme_mod( 'nandgatestudios-header-single-page', false );
            }

            // show or hide the header for archives templates
            else{
                $show_header = get_theme_mod( 'nandgatestudios-header-templates', false );
            }

            // header class
            $nandgatestudios_header_class      = 'nandgatestudios-miss-header-image';

            if( $show_header ){
                $nandgatestudios_header_class  = '';
            }
        ?>
        ...
        <header class="<?php echo esc_attr( $nandgatestudios_header_class ); ?>">

            <!-- header navigation -->
            <nav class="white nandgatestudios-topper" role="navigation">
                <div class="nav-wrapper container">

                    <!-- header button -->
                    <a href="#" data-activates="nav-mobile" class="button-collapse"><i class="nandgatestudios-icon-menu"></i></a>

                    <!-- Site Logo, Title and Description -->
                    <?php
                        $blog_title                     = esc_attr( get_bloginfo( 'name' ) );
                        $blog_description               = esc_attr( get_bloginfo( 'description' ) );
                        $nandgatestudios_logo_src              = esc_url( get_theme_mod( 'nandgatestudios-blog-logo' , get_template_directory_uri() . '/media/_frontend/img/logo.png' ) );

                        echo '<div class="nandgatestudios-blog-identity">';

                        if ( function_exists( 'the_custom_logo' ) && $has_custom_logo = has_custom_logo() ) {
                            echo '<div class="nandgatestudios-blog-logo" style="margin-top: ' . absint( get_theme_mod( 'nandgatestudios-blog-logo-m-top' ) ) . 'px; margin-bottom: ' . absint( get_theme_mod( 'nandgatestudios-blog-logo-m-bottom' ) ) . 'px;">';
                            the_custom_logo();
                            echo '</div>';
                        }

                        // blog logo ( if not is empty )
                        else if( !empty( $nandgatestudios_logo_src ) ){
                            echo '<a class="nandgatestudios-blog-logo" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '" style="margin-top: ' . absint( get_theme_mod( 'nandgatestudios-blog-logo-m-top' ) ) . 'px; margin-bottom: ' . absint( get_theme_mod( 'nandgatestudios-blog-logo-m-bottom' ) ) . 'px;">';
                            echo '<img src="' . esc_url( $nandgatestudios_logo_src ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '"/>';
                            echo '</a>';
                        }

                        // blog title ( if not is empty )
                        if( !empty( $blog_title ) ){
                            echo '<a class="nandgatestudios-blog-title" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '">';
                            bloginfo( 'name' );
                            echo '</a>';
                        }
                        ...
                        if( !empty( $blog_description ) ){
                            echo '<a class="nandgatestudios-blog-description" href="' . esc_url( home_url( '/' ) ) . '" title="' . esc_attr( $blog_title . ' - ' . $blog_description ) . '">';
                        ...
                        global $nandgatestudios_curr_ancestor;
                             ...
                             */
                                ...
                                if( !empty( $pages ) ){
                                    ...
                                    foreach( $pages as $p => $item ){
                                        $classes                = '';
                                        $nandgatestudios_curr_ancestor = false;
                                        ...
                                        $submenu = nandgatestudios_tools::submenu( $item -> ID );

                                        if( !empty( $submenu ) ){
                                            $classes .= ' menu-item-has-children';

                                            if( $nandgatestudios_curr_ancestor  ){
                                                $classes .= ' current-menu-ancestor';
                                            }
                                        }

                                        echo '<li class="menu-item ' . $classes . '">';
                                        echo '<a href="' . esc_url( get_permalink( $item -> ID ) ) . '" title="' . esc_attr( nandgatestudios_post::title( $item -> ID, true ) ) . '">' . nandgatestudios_post::title( $item -> ID ) . '</a>';
                                        echo $submenu;
                                        echo '</li>';
                                    }
                                    echo '</ul>';
