 ...
 */
	...
	add_action( 'admin_init', 			array( 'nandgatestudios_scripts', 	'backend' ) );

	add_action( 'admin_menu', 			array( 'nandgatestudios_admin', 	'pageMenu' ) );

	add_action( 'widgets_init', 		array( 'nandgatestudios_setup', 	'sidebars' ) );
	add_action( 'init', 				array( 'nandgatestudios_setup', 	'menus' ) );

	add_action( 'wp_enqueue_scripts',  array( 'nandgatestudios_scripts',  'frontend' ), 0 );
	add_action( 'wp_head', 				array( 'nandgatestudios_header', 	'head' ) );

    add_filter( 'the_excerpt_rss', 		array( 'nandgatestudios_tools', 	'rss_with_thumbnail' ) );
    add_filter( 'the_content_feed', 	array( 'nandgatestudios_tools', 	'rss_with_thumbnail' ) );

    if( get_theme_mod( 'nandgatestudios-gallery', true ) ){
        add_action( 'admin_init',           array( 'nandgatestudios_gallery',  'admin_init' ) );
        add_filter( 'post_gallery',         array( 'nandgatestudios_gallery',  'shortcode'), null, 2 );
    }
    ...
    add_image_size( 'nandgatestudios-classic' , 1140 , 515 , true );
